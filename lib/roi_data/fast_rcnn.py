...
def get_fast_rcnn_blob_names(is_training=True):
    ...
    if is_training:
        ...
        ##yxzhu's add
        #blob_names += ['polyinter_targets']
        #blob_names += ['polyinter_inside_weights']
        #blob_names += ['polyinter_outside_weights']
        ##pdb.set_trace()
        ######
...
def add_fast_rcnn_blobs_ohem(blobs, im_scales, roidb):
    """Add blobs needed for training Fast R-CNN style models."""
    # Sample training RoIs from each image and append them to the blob lists
    for im_i, entry in enumerate(roidb):
    ...
    for k, v in blobs.items():
        if isinstance(v, list) and len(v) > 0:
            blobs[k] = np.concatenate(v)
    ''' We use multilevel feature fusion like PANet, so no need to add_multilevel_rois
    # Add FPN multilevel training RoIs, if configured
    #if cfg.FPN.FPN_ON and cfg.FPN.MULTILEVEL_ROIS:
    #    _add_multilevel_rois(blobs)
    '''
...
def _all_rois(roidb, im_scale, batch_idx):
    ...
    if 'bbox_targets_ohem' not in roidb:
        #print('bbox_target not in')
        ...
        #pdb.set_trace()
        #print('bbox_target is in')
        bbox_targets, bbox_inside_weights = _expand_bbox_targets(
            roidb['bbox_targets_ohem'][keep_inds, :]
        )
    ...
    blob_dict = dict(
        labels_int32_ohem=sampled_labels.astype(np.int32, copy=False),
        rois_ohem=sampled_rois,
        bbox_targets_ohem=bbox_targets,
        bbox_inside_weights_ohem=bbox_inside_weights,
        bbox_outside_weights_ohem=bbox_outside_weights,
    )
    ''' Don't consider the mask branch when OHEM
...
def _sample_rois(roidb, im_scale, batch_idx):
    ...
    #add by yxzhu
    #sampled_polyinters=roidb['polyinters'][keep_inds]#add by yxzhu
    ...
    if 'bbox_targets' not in roidb:
        ...
        #gt_polyinters=roidb['polyinters'][gt_inds, :]#add by yxzhu
        #pdb.set_trace()
        gt_assignments = gt_inds[roidb['box_to_gt_ind_map'][keep_inds]]
        ...
        #polyinter_targets=_compute_targets_poly(sampled_boxes,gt_polyinters[gt_assignments, :], sampled_labels
        #)
        #polyinter_targets,polyinter_inside_weights=_expand_bbox_targets_poly(polyinter_targets)
        bbox_targets, bbox_inside_weights = _expand_bbox_targets(bbox_targets)
    else:
        #pdb.set_trace()
        #print('bbox_target is in')
    ...
    #polyinter_outside_weights=np.array(
    #    polyinter_inside_weights > 0, dtype=polyinter_inside_weights.dtype
    #)
    ...
    blob_dict = dict(
        ...
        #polyinter_targets=polyinter_targets,
        #polyinter_inside_weights=polyinter_inside_weights,
        #polyinter_outside_weights=polyinter_outside_weights,
    )
...
def _compute_targets_poly(ex_rois, gt_rois, labels):
    """Compute bounding-box regression targets for an image."""

    assert ex_rois.shape[0] == gt_rois.shape[0]
    assert ex_rois.shape[1] == 4
    assert gt_rois.shape[1] == 28

    targets = box_utils.bbox_transform_poly(
        ex_rois, gt_rois
    )/10.
    return np.hstack((labels[:, np.newaxis], targets)).astype(
        np.float32, copy=False
...
def _expand_bbox_targets_poly(bbox_target_data):
    """Bounding-box regression targets are stored in a compact form in the
    roidb.
    ...
    clss = bbox_target_data[:, 0]
    bbox_targets = blob_utils.zeros((clss.size, 28 * num_bbox_reg_classes))
    bbox_inside_weights = blob_utils.zeros(bbox_targets.shape)
    inds = np.where(clss > 0)[0]
    for ind in inds:
        cls = int(clss[ind])
