...
def build_generic_detection_model(
    ...
    def _single_gpu_build_func(model):
        ...
        if cfg.RPN.RPN_ON:
            ...
            #)
            if model.train:
                head_loss_gradients['rpn'] = rpn_heads.add_generic_rpn_outputs_for_ohem(
                    model, blob_conv, dim_conv, spatial_scale_conv
                )
            else:#testing time no OHEM
                head_loss_gradients['rpn'] = rpn_heads.add_generic_rpn_outputs(
                    model, blob_conv, dim_conv, spatial_scale_conv
                )
        ...
        if model.train:
            '''
            Do OHEM with the rois_ohem generated by rpn_heads.add_generic_rpn_outputs_for_ohem
            get the hard examples according to the losses, select the top cfg.TRAIN.BATCH_SIZE_PER_IM rois
            construct a minibatch to train the frcn head (and mask rcnn head)
            '''
            _add_fast_rcnn_read_only_head(
                model, add_roi_box_head_func, blob_conv, dim_conv,
                spatial_scale_conv
            )
            model.GenerateHardExamples()
            head_loss_gradients['box'] = _add_fast_rcnn_head_share_weights_with_ohem_head(
                model, add_roi_box_head_func, blob_conv, dim_conv,
                spatial_scale_conv
            )
...
def _add_fast_rcnn_read_only_head(
    model, add_roi_box_head_func, blob_in, dim_in, spatial_scale_in
):
    """Add a Fast R-CNN head to the model."""
    blob_frcn, dim_frcn = fast_rcnn_heads.add_roi_2mlp_ohem_head(
        model, blob_in, dim_in, spatial_scale_in, multilevel_fusion = True
    )
    fast_rcnn_heads.add_fast_rcnn_outputs_ohem(model, blob_frcn, dim_frcn)
    
def _add_fast_rcnn_head_share_weights_with_ohem_head(
    model, add_roi_box_head_func, blob_in, dim_in, spatial_scale_in
):
    """Add a Fast R-CNN head to the model."""
